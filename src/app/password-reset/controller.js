...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your ulavarservices account.<br>
        ...
        &mdash; ulavarservices Team
      `
      return sendMail({ toEmail: email, subject: 'ulavarservices - Password Reset', content })
    })
